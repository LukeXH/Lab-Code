...
function NatNetMatlabSample()
    ...
    addr = '::1';
    socket = tcpip(addr, 27015, 'NetworkRole', 'client');
    ...
    try
        ...
        LocalIP = char('169.254.190.36'); % Enter your local IP address
        ServerIP = char('169.254.30.125'); % Enter the server's IP address
    ...
    try
        if(newFrame)
            if(frameOfData.RigidBodies.Length() > 0)
                ...
                %%% Apply Controls
                % Current State
                q = [angleY; (angleY - angleY_last)/(frameTime - lastFrameTime)];
                % Desired Trajectory, square func from 0-90deg, period of 4
                % sec
                q_des = [0;0];
%                 q_des = [30*( 20 < mod(frameTime, 40) ) - 60 ; 0];
                % Raw cmd and scaled cmd, [255,0] is ccw, [0,255] is cw
                u = [0.05, 0.01]*(q - q_des);
                cmds = [fliplr(0:127), zeros(1,400-127);...
                        zeros(1,400-127), 0:127]';
%                 cmds = [zeros(1,192),   zeros(1,16), 64:255;... 
%                         fliplr(64:255), zeros(1,16), zeros(1,192);...
%                         fliplr(64:255), zeros(1,16), zeros(1,192);...
%                         zeros(1,192),   zeros(1,16), 64:255]';

                % Send the data
                ind = round(min(400, max( 399*(u+1)/2+1 , 1)));
                fwrite(socket, uint8( [0, cmds(ind,1), 0, cmds(ind,2)] ) );
